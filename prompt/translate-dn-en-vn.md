...
2.  **Dịch thuật:** Xác định các "câu gốc" (Câu được viết bằng tiếng Đan Mạch hoặc ngôn ngữ chính không phải tiếng Anh/Việt và không nằm trong ngoặc đơn) trong nội dung và dịch chúng sang tiếng Anh và tiếng Việt.

Điều quan trọng nhất là đảm bảo mỗi đơn vị thông tin (câu gốc, bản dịch tiếng Anh, bản dịch tiếng Việt) được hiển thị trên một hàng riêng biệt như một đoạn văn bản độc lập và tuân thủ tất cả các quy tắc định dạng được chỉ định.
...
2.  **Thực Hiện Dịch Thuật:**
      ...
      * Dịch mỗi "câu gốc" đã xác định sang **tiếng Việt**.
...
4.  **Xử Lý Bản Dịch Hiện Có:**
      ...
      * Nếu file Markdown đầu vào đã có sẵn bản dịch tiếng Anh hoặc/và tiếng Việt cho một câu gốc (thường nằm trong dấu ngoặc đơn ngay sau câu gốc), hãy:
          ...
          * Nếu một trong hai bản dịch (Anh hoặc Việt) bị thiếu, hãy **bổ sung bản dịch còn thiếu**.
          * Nếu cả hai đã có và đúng định dạng, giữ nguyên nhưng đảm bảo chúng tuân thủ quy tắc dòng trống.
          * Nếu cấu trúc hiện tại khác (ví dụ: `Câu gốc (English) (Vietnamese)` trên cùng một dòng), hãy **tách chúng ra** theo định dạng chuẩn ở mục 3.
...
2.  **Tiêu Đề (Headings H1-H6):**
      ...
      * **Không dịch tiêu đề.**
      * **QUY TẮC BỔ SUNG:** Nếu tiêu đề trong văn bản gốc sử dụng chữ cái để đánh số thứ tự (ví dụ: `## A. Tên tiêu đề`, `## B. Tiêu đề khác`), hãy **chuyển đổi chúng thành dạng số** (ví dụ: `## 1. Tên tiêu đề`, `## 2. Tiêu đề khác`). Áp dụng thay đổi này cho tiêu đề gốc trong file Markdown đầu ra.
...
5.  **Danh Sách (Lists):**
      ...
      * Cấu trúc ví dụ cho một mục danh sách:
        ```markdown
        1.  Det oprindelige indhold af listeelementet.

            (English translation of the list item's content.)

            (Bản dịch tiếng Việt của nội dung mục danh sách.)
...
**Ví dụ Chuyển Đổi (Bao gồm Dịch thuật):**
  ...
  * **Đầu vào có thể có:**
    ...
    ```markdown
    ## Et Afsnit
    Dette er den første sætning. Dette er den anden sætning. (This is the second sentence.)
  ...
  * **Đầu ra đã định dạng và dịch (mong muốn), đây là ví dụ:**
    ...
    De er glade for deres hus, og de nyder at have god plads.

    (They are happy with their house and they enjoy having plenty of space.)

    (Họ hài lòng với ngôi nhà của mình và thích có nhiều không gian.)
