Chắc chắn rồi\! Dưới đây là prompt mới đã được chỉnh sửa theo yêu cầu của bạn. Ngôn ngữ gốc giờ là **tiếng Anh**, và các ngôn ngữ đích là **tiếng Đan Mạch** và **tiếng Việt**. Toàn bộ cấu trúc, chức năng và các quy tắc định dạng đã được giữ nguyên.
...
2.  **Dịch thuật:** Xác định các "câu gốc" (Câu được viết bằng tiếng Anh và không nằm trong ngoặc đơn) trong nội dung và dịch chúng sang tiếng Đan Mạch và tiếng Việt.

Điều quan trọng nhất là đảm bảo mỗi đơn vị thông tin (câu gốc, bản dịch tiếng Đan Mạch, bản dịch tiếng Việt) được hiển thị trên một hàng riêng biệt như một đoạn văn bản độc lập và tuân thủ tất cả các quy tắc định dạng được chỉ định.
...
Một file `.md` có thể chưa được định dạng nhất quán. Một số câu gốc có thể chưa có bản dịch tiếng Đan Mạch hoặc tiếng Việt, hoặc có nhưng không theo chuẩn.
...
2.  **Thực Hiện Dịch Thuật:**
      ...
      * Dịch mỗi "câu gốc" (tiếng Anh) đã xác định sang **tiếng Đan Mạch**.
      * Dịch mỗi "câu gốc" (tiếng Anh) đã xác định sang **tiếng Việt**.

3.  **Định Dạng Kết Quả Dịch Thuật:**

      * Sau mỗi câu gốc, trình bày bản dịch tiếng Đan Mạch, tiếp theo là bản dịch tiếng Việt.
      * Bản dịch tiếng Đan Mạch phải được đặt trong dấu ngoặc đơn: `(Danish translation.)`
      * Bản dịch tiếng Việt phải được đặt trong dấu ngoặc đơn: `(Vietnamese translation.)`
      * **QUAN TRỌNG:** Mỗi phần (Câu gốc, Bản dịch tiếng Đan Mạch, Bản dịch tiếng Việt) phải được ngăn cách nhau bằng **một dòng trống hoàn toàn** trong file Markdown để đảm bảo mỗi phần hiển thị trên một hàng riêng và nhận kiểu của thẻ `<p>`.
      * Ví dụ cấu trúc chuẩn:
        ```markdown
        Lis and Bo live outside the city.

        (Lis og Bo bor uden for byen.)

        (Lis và Bo sống ở ngoại thành.)
        ```

4.  **Xử Lý Bản Dịch Hiện Có:**

      * Nếu file Markdown đầu vào đã có sẵn bản dịch tiếng Đan Mạch hoặc/và tiếng Việt cho một câu gốc (thường nằm trong dấu ngoặc đơn ngay sau câu gốc), hãy:
          ...
          * Nếu một trong hai bản dịch (Đan Mạch hoặc Việt) bị thiếu, hãy **bổ sung bản dịch còn thiếu**.
          * Nếu cả hai đã có và đúng định dạng, giữ nguyên nhưng đảm bảo chúng tuân thủ quy tắc dòng trống.
          * Nếu cấu trúc hiện tại khác (ví dụ: `Câu gốc (Danish) (Vietnamese)` trên cùng một dòng), hãy **tách chúng ra** theo định dạng chuẩn ở mục 3.
...
4.  **Văn Bản Trong Dấu Ngoặc Đơn `()` (Không phải bản dịch mới tạo):**
      ...
      * Nếu văn bản trong dấu ngoặc đơn trong file gốc không phải là bản dịch Đan Mạch/Việt của câu gốc ngay trước nó, hãy coi nó như một ghi chú độc lập và giữ nguyên. Áp dụng quy tắc "một câu trên một hàng" nếu nó cần đứng riêng.
...
5.  **Danh Sách (Lists):**
      ...
      * Cấu trúc ví dụ cho một mục danh sách:
        ```markdown
        1.  The original content of the list item.

            (Det oprindelige indhold af listeelementet.)

            (Nội dung gốc của mục danh sách.)
...
6.  **Trích Dẫn (Blockquotes):**
      ...
      * Mỗi dòng (câu gốc, dịch Đan Mạch, dịch Việt) trong trích dẫn phải bắt đầu bằng ` >  `.
...
**Ví dụ Chuyển Đổi (Bao gồm Dịch thuật):**
  ...
  * **Đầu vào có thể có:**
    ...
    ```markdown
    ## A Section
    This is the first sentence. This is the second sentence. (Dette er den anden sætning.)
    A third sentence without translation.
    * A list item.
    ```

  * **Đầu ra đã định dạng và dịch (mong muốn), đây là ví dụ:**
    ...
    (Một câu thứ ba không có bản dịch.)
...
Hãy xử lý file Markdown đầu vào và chỉ cung cấp nội dung Markdown đã được định dạng và dịch thuật lại theo các quy tắc trên.
